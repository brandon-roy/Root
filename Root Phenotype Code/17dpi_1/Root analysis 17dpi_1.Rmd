---
title: "GFLV Root Analysis 6.30.21 - Cohort ~2~"
author: "Brandon Roy"
date: "6/30/2021"
output:
...
Root_Summary <- read_excel("Root Summary.xlsx")
Root_Summary <- Root_Summary[-c(12,27),]
Root_Summary <- Root_Summary %>% mutate(treatment = fct_relevel(treatment, "control", "wildtype F13", "mutant F13 1E G802K", "wildtype GHu", "mutant GHu 1E K802G"))
metadata <- read.delim("FanleafMetadata.txt", sep="\t", header = TRUE)
```
...
Root_Summary %>%
  ...
  summarise(
    ...
    `dry biomass (g)` = mean(`dry biomass (g)`),
...
ggdensity(Root_Summary, x = "average diameter (mm)", y= '..count..', facet.by = "treatment")
...
mod_means <- multcomp::cld(object = mod_means_contr$emmeans,
                           Letters = letters)
# median diameter
mod <- lm(`median diameter (mm)` ~ treatment, data = Root_Summary)
mod_means_contr <- emmeans::emmeans(object = mod,
                                    pairwise ~ "treatment",
                                    adjust = "tukey")
mod_means <- multcomp::cld(object = mod_means_contr$emmeans,
...
mod_means <- multcomp::cld(object = mod_means_contr$emmeans,
                           Letters = letters)
# branching frequency
mod <- lm(`branching frequency (mm^-1)` ~ treatment, data = Root_Summary)
mod_means_contr <- emmeans::emmeans(object = mod,
                                    pairwise ~ "treatment",
                                    adjust = "sidak")
mod_means <- multcomp::cld(object = mod_means_contr$emmeans,
...
```{r}
#Height
Height_plot <- ggboxplot(metadata, x = 'treatment', y= 'height..cm.', fill = "treatment", palette = colorlist3, xlab = FALSE, width = 0.5, ylab = "height (cm)") +
  stat_compare_means(label = 'p.signif', method ='t.test', ref.group = "control", hide.ns = TRUE)
#Total Biomass
Total_mass_plot <- ggboxplot(metadata, x = "treatment", y= "Plant.mass..g.", fill = "treatment", palette = colorlist3, xlab = FALSE, ylab = "biomass (g)", width = 0.5, )+
  stat_compare_means(label = 'p.signif', method ='t.test', ref.group = "control", hide.ns = TRUE)
#Dry Biomass
Dry_mass_plot <- ggboxplot(metadata, x = "treatment", y= "dry.biomass..g.", fill = "treatment", palette = colorlist3, xlab = FALSE, width = 0.5, ylab = "dry biomass (g)")+
  stat_compare_means(label = 'p.signif', method ='anova', ref.group = "control", hide.ns = TRUE)
#Volume
Vol_plot <- ggboxplot(Root_Summary, x = "treatment", y= "volume", fill = "treatment", palette = colorlist3, xlab = FALSE, width = 0.5, ylab = "volume (mm^3)")+
...
multi_plot2 <- ggarrange(Height_plot + rremove("x.text"), Total_mass_plot + rremove("x.text"), Dry_mass_plot + rremove("x.text"), Vol_plot + rremove("x.text"), common.legend = TRUE, legend = 'bottom',labels = 'AUTO')
multi_plot2
...
ggboxplot(Root_Summary, x = "treatment", y= "total root length (mm)")
...
number_of_samples <- c(14,15,15,15,15)
...
ggviolin(Root_Summary, x = "treatment", y= "number of branch points", fill = "treatment", palette =colorlist) + 
  geom_boxplot(width = .2, alpha = .7, fatten = NULL) +
  stat_compare_means(aes(label = ..p.signif..), method="t.test",ref.group = "control", hide.ns = T, label.y = c(1300,1200,1100,1300,1300), bracket.size = 0.3) +
  stat_summary(fun = "mean", geom = "point") +
  stat_summary(fun.data = "mean_se", geom = "errorbar", width = .1) + 
  stat_compare_means(method="anova",label.y=1935, label.x=1.3, size=5) +
  theme(aspect.ratio = 1) +
  theme(axis.text.x = element_text(''), legend.position = 'NA')+theme(text=element_text(size=15))
...
ggviolin(Root_Summary, x = "treatment", y= "height (mm)", fill = "treatment", palette =colorlist) + 
  geom_boxplot(width = .2, alpha = .7, fatten = NULL) +
  stat_compare_means(aes(label = ..p.signif..), method="t.test",ref.group = "control", hide.ns = T, label.y = c(300,175,250,250,182), bracket.size = 0.3) +
  stat_summary(fun = "mean", geom = "point") +
  stat_summary(fun.data = "mean_se", geom = "errorbar", width = .1) + 
  stat_compare_means(method="anova",label.y=210, size = 5) +
  theme(aspect.ratio = 1) +
  theme(axis.text.x = element_text(''), legend.position = 'NA')+theme(text=element_text(size=15))

ggviolin(Root_Summary, x = "treatment", y= "dry biomass (g)", fill = "treatment", palette =colorlist) + 
  geom_boxplot(width = .2, alpha = .7, fatten = NULL) +
  stat_compare_means(aes(label = ..p.signif..), method="t.test",ref.group = "control", hide.ns = T, label.y = c(7.5,7.5,7.5,7.5,7.5), bracket.size = 0.3) +
  stat_summary(fun = "mean", geom = "point") +
  stat_summary(fun.data = "mean_se", geom = "errorbar", width = .1) + 
  stat_compare_means(method="anova",label.y=10, size = 5) +
  theme(aspect.ratio = 1) +
  theme(axis.text.x = element_text(''), legend.position = 'NA')+theme(text=element_text(size=15))

ggviolin(Root_Summary, x = "treatment", y= "network area (mm^2)", fill = "treatment", palette =colorlist) + 
  geom_boxplot(width = .2, alpha = .7, fatten = NULL) +
  stat_compare_means(aes(label = ..p.signif..), method="t.test",ref.group = "control", hide.ns = T, label.y = c(1200,1000,800,830,1100), bracket.size = 0.3) +
  stat_summary(fun = "mean", geom = "point") +
  stat_summary(fun.data = "mean_se", geom = "errorbar", width = .1) + 
  stat_compare_means(method="anova",label.y=1250, size = 5) +
  theme(aspect.ratio = 1) +
  theme(axis.text.x = element_text(''), legend.position = 'NA')+theme(text=element_text(size=15))

ggviolin(Root_Summary, x = "treatment", y= "surface area (mm^2)", fill = "treatment", palette =colorlist) + 
  geom_boxplot(width = .2, alpha = .7, fatten = NULL) +
  stat_compare_means(aes(label = ..p.signif..), method="t.test",ref.group = "control", hide.ns = T, label.y = c(1200,5000,5000,4500,5000), bracket.size = 0.3) +
  stat_summary(fun = "mean", geom = "point") +
  stat_summary(fun.data = "mean_se", geom = "errorbar", width = .1) + 
  stat_compare_means(method="anova",label.y=6000, size = 5) +
  theme(aspect.ratio = 1) +
  theme(axis.text.x = element_text(''), legend.position = 'NA')+theme(text=element_text(size=15))

ggviolin(Root_Summary, x = "treatment", y= "perimeter (mm)", fill = "treatment", palette =colorlist) + 
  geom_boxplot(width = .2, alpha = .7, fatten = NULL) +
  stat_compare_means(aes(label = ..p.signif..), method="t.test",ref.group = "control", hide.ns = T, label.y = c(4000,3200,3100,3100,4000), bracket.size = 0.3) +
  stat_summary(fun = "mean", geom = "point") +
  stat_summary(fun.data = "mean_se", geom = "errorbar", width = .1) + 
  stat_compare_means(method="anova",label.y=4500, size = 5) +
  theme(aspect.ratio = 1) +
  theme(axis.text.x = element_text(''), legend.position = 'NA')+theme(text=element_text(size=15))
...
ggviolin(Root_Summary, x = "treatment", y= "volume (mm^3)", fill = "treatment", palette =colorlist) + 
  ...
  stat_summary(fun = "mean", geom = "point") +
  stat_summary(fun.data = "mean_se", geom = "errorbar", width = .1) + 
  stat_compare_means(method="anova",label.y=2500, size = 5) +
  theme(aspect.ratio = 1) +
  theme(axis.text.x = element_text(''), legend.position = 'NA')+theme(text=element_text(size=15))
```
...
#Height
he <- ggboxplot(Root_Summary, x = "treatment", y= "Height (cm)", fill = "treatment", palette = "jco", xlab = FALSE, width = 0.5, )+
  stat_compare_means(label = 'p.signif', method ='anova', ref.group = "Control", hide.ns = TRUE)
he
#Volume
rtvol <- ggboxplot(Root_Summary, x = "treatment", y= "Volume", fill = "treatment", palette = "jco", xlab = FALSE, width = 0.5, )+
  stat_compare_means(label = 'p.signif', method ='t.test', ref.group = "Control", hide.ns = TRUE)
#Network Area
rtnet <- ggboxplot(Root_Summary, x = "treatment", y= "Network area (mm^2)", fill = "treatment", palette = "jco", xlab = FALSE, width = 0.5, )+
  stat_compare_means(label = 'p.signif', method ='t.test', ref.group = "Control", hide.ns = TRUE)
#Surface Area
rtsa <- ggboxplot(Root_Summary, x = "treatment", y= "Surface area", fill = "treatment", palette = "jco", xlab = FALSE, width = 0.5, )+
  stat_compare_means(label = 'p.signif', method ='t.test', ref.group = "Control", hide.ns = TRUE)

#combine everything together for a 4-panel figure
multi_plot2 <- ggarrange(he + rremove("x.text"), rtvol + rremove("x.text"), rtnet + rremove("x.text"), rtsa + rremove("x.text"), Total_mass_plot + rremove("x.text"), common.legend = TRUE, legend = 'bottom',labels = 'AUTO')
multi_plot2

# This function may cause R to crash... 
# ggsave("Otherplot.tiff",multi_plot2)
```

```{r}
root_sum_matrix <- Root_Summary[3:19]

root_cor <- cor(root_sum_matrix, use='complete.obs')

res1 = cor.mtest(root_cor, conf.level=0.99)
corrplot(root_cor, type = 'full', order='hclust', hclust.method='average', tl.col='black', tl.srt=45, addrect=6, method="circle", p.mat = res1$p, sig.level = c(0.001, 0.01, 0.05), pch.cex = 0.9, insig = 'label_sig', pch.col = 'grey20')
